...
from logic import get_answer, get_survey_area, get_survey_question, get_question, get_session, get_user, parce_questions, create_session_code, get_survey, parce_survey_area, parce_surveys, parce_users
    ...
    if session is None:
        ...
        if question is None:
            ...
            if survey_question is None:
                ...
                return HttpResponseForbidden('You cannot delete a question related to surveys')
    ...
    if session is None:
        ...
        if answer is None:
            ...
            if survey_question is None:
                ...
                return HttpResponseForbidden('You cannot delete a reply used by other people ')
...
def edit_survey(request):
    ...
    if session is None:
        ...
        if survey is None:
            ...
            if survey.author_id == session.user_id:
                survey = Survey.objects.filter(id=body['survey_id']).update(name=body['name'])
                survey = Survey.objects.filter(id=body['survey_id']).update(type=body['type'])
